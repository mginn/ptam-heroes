...
int wh4=0;
  ...
  cout<< "Loop itration at line 121 mapMaker.cc wh4 "<<wh4<<endl;
}
...
void MapMaker::HandleBadPoints()
{
  // Did the tracker see this point as an outlier more often than as an inlier?
	//cout<< "Loop itration at line 154 MapMaker.cc mMap.vpPoints.size()"<<mMap.vpKeyFrames.size()<<endl;
//cout<< "Loop itration at line 141 MapMaker.cc mMap.vpPoints.size()"<<mMap.vpPoints.size()<<endl;
			      ...
			      SE3<> &se3TrackerPose)
{
 ...
 // cout<< "Loop itration at line 216 Mapviewer.cc vTrailMatches.size()"<<vTrailMatches.size()<<endl;
  ...
  *pkSecond = kS;
  ...
  PatchFinder finder;
  //cout<< "Loop itration at line 260 mapMaker.cc vMatches.size()"<<vMatches.size()<<endl;
 ...
 // cout<< "Loop itration at line 337 mapMaker.cc wh2 "<<wh2<<endl;
 // cout<< "Time taken at 337 mapMaker.cc wh2 "<<(i348-i340)<<endl;
  ...
  //edited by mainul for outputting to file
  FILE *fpi1 = fopen("messg_f_usr_mapmkr.txt", "w");
...
void MapMaker::ThinCandidates(KeyFrame &k, int nLevel)
{
  ...
  //cout<< "Loop itration at line 368 mapMaker.cck.mMeasurements.begin(),k.mMeasurements.end()"<<std::distance(k.mMeasurements.begin(),k.mMeasurements.end())<<endl;
  ...
  //cout<< "Loop itration at line 381 mapMaker.cc vCSrc.size()"<<vCSrc.size()<<endl;
  //cout<< "Loop itration at line 385 mapMaker.cc vCSrc.size()"<<irBusyLevelPos.size()<<endl;
...
void MapMaker::AddSomeMapPoints(int nLevel)
{
  ...
  //cout<< "Loop itration at line 413 mapMaker.cc l.vCandidates.size()"<<l.vCandidates.size()<<endl;
  ...
  for(unsigned int i = 0; i<l.vCandidates.size(); i++)
  {
    ...
    double i2=get_wall_time();//mnl>>chekce the time

    //cout<<"Time for single iteration of epipolar Search:"<<(i2-i1)<<endl;
  }
};
...
void MapMaker::ApplyGlobalTransformationToMap(SE3<> se3NewFromOld)
{
	 ...
	 //cout<< "Loop itration at line 430 mapMaker.cc mMap.vpKeyFrames.size()"<<mMap.vpKeyFrames.size()<<endl;
	 //cout<< "Loop itration at line 434 mapMaker.cc mmMap.vpPoints.size()"<<mMap.vpPoints.size()<<endl;
...
{
	// cout<< "Loop itration at line 449 mapMaker.cc mMap.vpKeyFrames.size()"<<mMap.vpKeyFrames.size()<<endl;
	// cout<< "Loop itration at line 452 mapMaker.cc mMap.vpPoints.size()"<<mMap.vpPoints.size()<<endl;
...
void MapMaker::AddKeyFrameFromTopOfQueue()
{
  ...
  //cout<< "Loop itration at line 489 mapMaker.cc pK->mMeasurements.end()"<<std::distance(pK->mMeasurements.begin(),pK->mMeasurements.end())<<endl;
				...
				int nCandidate)
{
  ...
  if(!kTarget.aLevels[nLevel].bImplaneCornersCached)
    {
	//  cout<< "Loop itration at line 590 mapMaker.cc vIR.size()"<<vIR.size()<<endl;
  ...
  //cout<< "Loop itration at line 602 mapMaker.cc vv2Corners.size()"<<vv2Corners.size()<<endl;
  double i605=get_wall_time();
  ...
  double i620=get_wall_time();


//  cout<<"Time for prospective loops line 602 mapmaker opencl: "<<(i620-i605)<<endl;
...
vector<KeyFrame*> MapMaker::NClosestKeyFrames(KeyFrame &k, unsigned int N)
{
  ...
  //cout<< "Loop itration at line 684 mapMaker.cc mMap.vpKeyFrames.size()"<<mMap.vpKeyFrames.size()<<endl;
  ...
  //cout<< "Loop itration at line 699 mapMaker.cc mMap.N "<<N<<endl;
...
KeyFrame* MapMaker::ClosestKeyFrame(KeyFrame &k)
{
  ...
  //cout<< "Loop itration at line 711 mapMaker.cc mMap.vpKeyFrames.size() "<<mMap.vpKeyFrames.size()<<endl;
...
void MapMaker::BundleAdjustAll()
{
  ...
  //cout<< "Loop itration at line 755 mapMaker.cc mMap.vpKeyFrames.size() "<<mMap.vpKeyFrames.size()<<endl;
  //cout<< "Loop itration at line 762 mapMaker.cc mMap.vpPoints.size() "<<mMap.vpPoints.size()<<endl;
  for(unsigned int i=0; i<mMap.vpKeyFrames.size(); i++)
  ...
  double i774=get_wall_time();
  for(unsigned int i=0; i<mMap.vpPoints.size();i++)
    sMapPoints.insert(mMap.vpPoints[i]);
  double i777=get_wall_time();
  //cout<<"Time for prospective 778 loops Mapmaker opencl: "<<(i777-i774)<<endl;
  double i779=get_wall_time();
  BundleAdjust(sAdj, sFixed, sMapPoints, false);
  double i781=get_wall_time();
 // cout<<"Time for prospective BundleAdjust 780 loops Mapmaker opencl: "<<(i781-i779)<<endl;
...
void MapMaker::BundleAdjustRecent()
{
 ...
 // cout<< "Loop itration at line 794 mapMaker.cc sAdjustSet.begin() "<<std::distance(sAdjustSet.begin(),sAdjustSet.end())<<endl;
  ...
  for(set<KeyFrame*>::iterator iter = sAdjustSet.begin();
      ...
      iter++)
    {
      ...
      cout<< "Loop itration at line 800 mapMaker.cc mKFMeas.begin()"<<std::distance(mKFMeas.begin(),mKFMeas.end())<<endl;
 ...
 // cout<< "Loop itration at line 809 mapMaker.cc vMatches.size()"<<std::distance(mMap.vpKeyFrames.begin(),mMap.vpKeyFrames.end())<<endl;
  ...
  for(vector<KeyFrame*>::iterator it = mMap.vpKeyFrames.begin(); it!=mMap.vpKeyFrames.end(); it++)
	...
	continue;
      bool bInclude = false;
    //  cout<< "Loop itration at line 815 mapMaker.cc (*it)->mMeasurements.end()"<<std::distance((*it)->mMeasurements.begin(),(*it)->mMeasurements.end())<<endl;
...
void MapMaker::BundleAdjust(set<KeyFrame*> sAdjustSet, set<KeyFrame*> sFixedSet, set<MapPoint*> sMapPoints, bool bRecent)
{
  ...
  //cout<< "Loop itration at line 847 mapMaker.cc sAdjustSet.end() "<<std::distance(sAdjustSet.begin(),sAdjustSet.end())<<endl;
  //cout<< "Loop itration at line 853 mapMaker.cc sAdjustSet.end() "<<std::distance(sFixedSet.begin(),sFixedSet.end())<<endl;
 ...
 // cout<< "Loop itration at line 871 mapMaker.cc mMap.vpKeyFrames.size() "<<mMap.vpKeyFrames.size()<<endl;
  //cout<< "Loop itration at line 863 mapMaker.cc sAdjustSet.end() "<<std::distance(sMapPoints.begin(),sMapPoints.end())<<endl;
  double i881=get_wall_time();
  ...
  double i889=get_wall_time();
  //cout<<"Mapmaker inside BundleAdjust prospective line 863 opencl candidate: "<<(i889-i881)<<endl;
  // Add the relevant point-in-keyframe measurements

  double i893=get_wall_time();
  for(unsigned int i=0; i<mMap.vpKeyFrames.size(); i++)
    {
	...
	continue;
     ...
     // cout<< "Loop itration at line 880 mapMaker.cc mMap.vpKeyFrames["<<i<<"]->mMeasurements.end() "<<std::distance(mMap.vpKeyFrames[i]->mMeasurements.begin(),mMap.vpKeyFrames[i]->mMeasurements.end())<<endl;
  ...
  //cout<<"Mapmaker inside BundleAdjust prospective opencl candidate loop at 894: "<<(i914-i893)<<endl;
  // Run the bundle adjuster. This returns the number of successful iterations
  double i917=get_wall_time();
 // cout<<"calling Compute inside BundleAdjust function.................";
  int nAccepted = b.Compute(&mbBundleAbortRequested);
  double i919=get_wall_time();
// cout<<"Mapmaker inside BundleAdjust Compute opencl candidate loop at 917: "<<(i919-i917)<<endl;
  ...
  if(nAccepted > 0)
    {
	 ...
	 // cout<< "Loop itration at line 911 mapMaker.cc mPoint_BundleID.begin() "<<std::distance(mPoint_BundleID.begin(),mPoint_BundleID.end())<<endl;
	 // cout<< "Loop itration at line 916 mapMaker.cc mPoint_BundleID.begin() "<<std::distance(mView_BundleID.begin(),mView_BundleID.end())<<endl;
  ...
  //cout<< "Loop itration at line 942 mapMaker.cc vOutliers_PC_pair.size()"<<vOutliers_PC_pair.size()<<endl;
...
bool MapMaker::ReFind_Common(KeyFrame &k, MapPoint &p)
{
  ...
  bool bFound = Finder.FindPatchCoarse(ir(v2Image), k, 4,1);  // Very tight search radius!
  if(!bFound)
    {
      p.pMMData->sNeverRetryKFs.insert(&k);
      return false;
...
int MapMaker::ReFindInSingleKeyFrame(KeyFrame &k)
{
  ...
  //cout<< "Loop itration at line 1054 mapMaker.cc mMap.vpPoints.size() "<<mMap.vpPoints.size()<<endl;
 // cout<< "Loop itration at line 1058 mapMaker.cc vToFind.size() "<<vToFind.size()<<endl;
  for(unsigned int i=0; i<mMap.vpPoints.size(); i++)
...
void MapMaker::ReFindNewlyMade()
{
  ...
  //cout<< "Loop itration at line 1088 mapMaker.cc mMap.vpKeyFrames.size()"<<mMap.vpKeyFrames.size()<<endl;
int wh1=0;
 ...
 // cout<< "Loop itration at line 1079 mapMaker.cc wh1 "<<wh1<<endl;
};
...
void MapMaker::ReFindFromFailureQueue()
{
  ...
  //cout<< "Loop itration at line 1106 mapMaker.cc vMatches.size()"<<std::distance(mvFailureQueue.begin(),mvFailureQueue.end())<<endl;
...
SE3<> MapMaker::CalcPlaneAligner()
{
 ...
 // cout<< "Loop itration at line 1136 mapMaker.cc nRansacs"<<nRansacs<<endl;
  ...
  for(int i=0; i<nRansacs; i++)
	...
	continue;
     ...
     // cout<< "Loop itration at line 1162 mapMaker.cc nPoints "<<nPoints<<endl;
...
//  cout<< "Loop itration at line 1188 mapMaker.cc nPoints "<<nPoints<<endl;
 ...
 // cout<< "Loop itration at line 1204 mapMaker.cc vv3Inliers.size() "<<vv3Inliers.size()<<endl;
...
void MapMaker::RefreshSceneDepth(KeyFrame *pKF)
{
 ...
 // cout<< "Loop itration at line 1247 mapMaker.cc mMeasurements.begin() "<<std::distance(pKF->mMeasurements.begin(),pKF->mMeasurements.end())<<endl;
...
void MapMaker::GUICommandHandler(string sCommand, string sParams)  // Called by the callback func..
{
  if(sCommand=="SaveMap")
   ...
   //   cout<< "Loop itration at line 1278 mapMaker.cc mMap.vpPoints.size()"<<mMap.vpPoints.size()<<endl;
    //  cout<< "Loop itration at line 1285 mapMaker.cc mMap.vpKeyFrames.size()"<<mMap.vpKeyFrames.size()<<endl;
