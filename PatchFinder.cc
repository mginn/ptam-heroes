					      ...
					      Matrix<2> &m2CamDerivs)
{
  ...
  //checking the value of se3CFromW
  //cout<<"checking SE3 : "<<se3CFromW<<endl;
  ...
  mm2WarpInverse.T()[0] = m2CamDerivs * (v3MotionRight.slice<0,2>() - v3Cam.slice<0,2>() * v3MotionRight[2] * dOneOverCameraZ) * dOneOverCameraZ; //mnl>>here is some matrix multiplication. We can use OpenCL here.
  ...
  while(dDet > 3 && mnSearchLevel < LEVELS-1)
    {
      mnSearchLevel++;
      dDet *= 0.25;			//mnl>>need to check dDet initial value to be sure about loop iteration's count
    };
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &p)
{
  ...
  Matrix<2> m2 = M2Inverse(mm2WarpInverse) * LevelScale(mnSearchLevel);  //mnl>>that means a 2x2 matrix
  // m2 now represents the number of pixels in the source image for one 
  // pixel of template image
  

  /*cout<<"warpmatrix--input:"<<mm2LastWarpMatrix<<endl;
  cout<<"output matrix: "<<m2<<endl;
  cout<<"indexed t m2 matrix:"<<m2.T()[0]<<endl;
  cout<<"indexed mm2LastWarpMatrix matrix:"<<m2.T()[0] - mm2LastWarpMatrix.T()[0]<<endl ;*/
  //cout<<"indexed t m2 matrix:"<<m2.T()[1];
  ...
  for(int i=0; !bNeedToRefreshTemplate && i<2; i++)
    {
      Vector<2> v2Diff = m2.T()[i] - mm2LastWarpMatrix.T()[i]; //mnl>>T() it gives the corresponding ith columns. this line subtracts values of jth element of ith column
...
inline void PatchFinder::MakeTemplateSums()
{
  ...
  //cout<< "Loop itration at line 180 PatchFinder.cc cnt_loop"<<cnt_loop<<endl;
...
bool PatchFinder::FindPatchCoarse(ImageRef irPos, KeyFrame &kf, unsigned int nRange,int iter)
{
  mbFound = false;
  ...
  /*cout<<"iterator ix value: "<<i->x<<endl;
  cout<<"iterator iy value: "<<i->y<<endl;
  cout<<"vCornerRowLUT  value: "<<L.vCornerRowLUT[nTop]<<endl;

  cout<<"begin....value x:"<<L.vCorners.begin()->x<<endl;
 cout<<"begin....value y:"<<L.vCorners.begin()->y<<endl;

  cout<<"array index....value x:"<<L.vCorners[0].x<<endl;
  cout<<"array index....value y:"<<L.vCorners[0].y<<endl;

  cout<<"array index with lut value....value x:"<<L.vCorners[L.vCornerRowLUT[nTop]].x<<endl;
  cout<<"array index with lut value....value y:"<<L.vCorners[L.vCornerRowLUT[nTop]].y<<endl;*/
  ...
  //cout<< "Loop itration at line 256 PatchFinder.cc i_end"<<std::distance(i,i_end)<<endl;
  double i260=get_wall_time();
  for(; i<i_end; i++)          // For each corner ...//mnl only this portion canbe transferredf to opencl
  {
  ...
  double i281=get_wall_time();

  //cout<<"Time for prospective loops PathchFinder opencl: "<<(i281-i260)<<endl;

  if(nBestSSD < mnMaxSSD)      // Found a valid match?
    {
      mv2CoarsePos= LevelZeroPos(irBest, mnSearchLevel); //mnl>>imgref takes integer but this function takes double, also returns double but assiging value to double.
      mbFound = true;
    }
  else
	  mbFound = false;
  if(iter==4)
  {
	  // printf("inside CPU patchfinder: ....\n");
	  //printf(" nRange....%d\n",nRange);
	  //printf("nLeft ....%d\n",nLeft);
	  //printf("mv2CoarsePos CPU x: ....%f\n",mv2CoarsePos[0]);
	  //printf("irpos value x; ....%f\n",irPos.x);
	//  printf("mv2CoarsePos CPU y....%f\n",mv2CoarsePos[1]);

	  //printf("iteration: ....%d\n",c);
	  //printf("nBestSSD ....%d\n",nBestSSD);
	  //printf("mnmaxssd ....%d\n",mnMaxSSD);
	  //printf("irBest_Y....%d\n",irBest.y);
	  //printf("maqsquared value ....%d\n",(irPos - *(i_end-1)).mag_squared() );
	  //printf("L->vCorners[k][0] ....%d\n",(i_end-1)->x);
	  //printf("inside CPU patchfinder  ends: ....\n");
  }
...
void PatchFinder::MakeSubPixTemplate()
{
  ...
  //cout<< "Loop itration at line 319 PatchFinder.cc i_end"<<mnPatchSize - 1<<endl;
...
bool PatchFinder::IterateSubPixToConvergence(KeyFrame &kf, int nMaxIts)
{
  ...
  //cout<< "Loop itration at line 354 PatchFinder.cc nMaxIts"<<nMaxIts<<endl;
...
double PatchFinder::IterateSubPix(KeyFrame &kf)
{
 ...
 // cout<< "Loop itration at line 403 PatchFinder.cc mnPatchSize"<<mnPatchSize-1<<endl;
...
if(!im.in_image_with_border(ir, mirCenter[0]))
  ...
  //mnl>> this is the way how we can convert image to basic data type
  unsigned char *testimage=im.data();
  unsigned char *mnTemp=mimTemplate.data();
  /*int a=testimage[1];
  cout<<"in zxmssssss......"<<a<<endl;
  unsigned char *mnTemp=mimTemplate.data();*/

  int stride=im.row_stride();
  int strTemp=mimTemplate.row_stride();
  int my_size[2];
  my_size[0]=im.size().x;


 /* cout<<"image stride: "<<stride<<endl;
  cout<<"template stride: "<<strTemp<<endl;
  cout<<"rImgBase.y >"<< irImgBase.y<<endl;
  cout<<"irImgBase.x >"<< irImgBase.x<<endl;
  cout<<"irImgBase.y+nRow*stride+(irImgBase.x + 0)"<<irImgBase.y+0*stride+(irImgBase.x + 0)<<endl;
  cout<<"testimage[desired index]"<<s<<endl;*/
...
#endif 
  {
	...
	//  cout<< "Loop itration at line 635 PatchFinder.cc mnPatchSize"<<mnPatchSize<<endl;
    //mnl>>below code was written to test that my strided array access to unsigned character array works..and it works
	  for(int nRow = 0; nRow < mnPatchSize; nRow++)
	 	{
	       //imagepointer = &testimage[(irImgBase.y+nRow)*stride+(irImgBase.x + 0)];

	 	  //templatepointer = &mnTemp[nRow*strTemp + 0];
	 	 // cout<<"index values coord: "<<irImgBase + ImageRef(0,nRow);
	 	 // cout<<"index values int: "<<(irImgBase + ImageRef(0,nRow)).y*stride+(irImgBase + ImageRef(0,nRow)).x<<endl;
	 	 // cout<<"stride values: "<<stride<<endl;
	 	 // int y=imagepointer[0];
	 	 // cout<<"Array values: "<<y<<endl;
	 	 // int k=testimage[(irImgBase + ImageRef(0,nRow)).y*stride+(irImgBase + ImageRef(0,nRow)).x];
	 	 // cout<<"image value: "<<k<<endl;

	 	  /*for(int x=0;x<mnPatchSize;x++)
	 	  {
	 		  int y=imagepointer[x];
	 		 	  cout<<"imagepointer[0]......."<<x<<"...."<<y<<endl;
	 	  }*/


	 	 for(int nCol = 0; nCol < mnPatchSize; nCol++)
	 	    {
	 	      int n = testimage[(irImgBase.y+nRow)*stride+(irImgBase.x + 0)+nCol];//imagepointer[nCol];
	 	      nImageSum += n;
	 	      nImageSumSq += n*n;
	 	      nCrossSum += n * mnTemp[nRow*strTemp + 0+nCol];//templatepointer[nCol];
	  	...
	  	//cout<<"mnTemplateSum CPU: "<<mnTemplateSum<<endl;
	    //cout<<"mnTemplateSumSq CPU: "<<mnTemplateSumSq<<endl;
	   /* cout<<"rImgBase.y >"<< irImgBase.y<<endl;
	    cout<<"irImgBase.x >"<< irImgBase.x<<endl;
	    cout<<"irImgBase.y+nRow*stride+(irImgBase.x + 0)"<<irImgBase.y+0*stride+(irImgBase.x + 0)<<endl;
	    cout<<"testimage[desired index]"<<s<<endl*/
